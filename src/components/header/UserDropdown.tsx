...
const roleGradients: Record<UserRole, { color: string; gradient: string }> = {
  operator: { color: "bg-success-500", gradient: "from-success-400 to-success-600" },
  administrator: { color: "bg-brand-500", gradient: "from-brand-400 to-brand-600" },
  user: { color: "bg-orange-500", gradient: "from-orange-400 to-orange-600" },
};
...
export default function UserDropdown() {
  ...
  const currentRole = roleGradients[user.role] || roleGradients.administrator;
  ...
  return (
    <div className="relative">
      ...
      >
        <span className={`flex items-center justify-center overflow-hidden rounded-xl h-10 w-10 bg-gradient-to-br ${currentRole.gradient} text-white text-sm font-semibold shadow-md shadow-brand-500/20`}>
          {initials}
        </span>
      ...
      >
        ...
        <div className="flex items-center gap-3 pb-4 border-b border-gray-100 dark:border-gray-800">
          <span className={`flex items-center justify-center rounded-xl h-12 w-12 bg-gradient-to-br ${currentRole.gradient} text-white font-semibold shadow-md`}>
            {initials}
          </span>
