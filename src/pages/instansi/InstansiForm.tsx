...
export default function InstansiForm() {
  ...
  useEffect(() => {
    if (isEdit && id) {
      setLoading(true);
      instansiService.getById(id)
        .then((data) => { setNama(data.nama); setDeskripsi(data.deskripsi || ""); })
        .catch(() => { alert("Data tidak ditemukan"); navigate("/instansi"); })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit && id) await instansiService.update(id, payload);
      else await instansiService.create(payload);
      navigate("/instansi");
  ...
  return (
    <FormPage title="Instansi" isEdit={isEdit} onSubmit={handleSubmit} onCancel={() => navigate("/instansi")} saving={saving} loading={loading}>
      <FormField label="Nama Instansi" htmlFor="nama" required error={errors.nama}>
        <Input id="nama" type="text" placeholder="Masukkan nama instansi" value={nama}
          onChange={(e) => { setNama(e.target.value); setErrors((p) => ({ ...p, nama: "" })); }} />
      </FormField>
      <FormField label="Deskripsi" htmlFor="deskripsi">
        <textarea id="deskripsi" rows={4} placeholder="Masukkan deskripsi instansi" value={deskripsi}
