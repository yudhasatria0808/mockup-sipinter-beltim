...
export default function PotensiKonflikList() {
  ...
  const columns: DataTableColumn<PotensiKonflik>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <span className="line-clamp-2 max-w-xs">{row.namaPotensiKonflik}</span>
      ),
    },
    {
      key: "aspek",
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-end gap-1">
          <button
            onClick={() => navigate(`/potensi-konflik/${row.id}`)}
          ...
          {row.status === "draft" && (
            <>
              <button
                onClick={() => navigate(`/potensi-konflik/edit/${row.id}`)}
                className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800"
                title="Edit"
              >
                <EditIcon />
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-48">
            ...
            <Input
              placeholder="Cari nama konflik, wilayah..."
              value={search}
