...
export default function LevelDampakForm() {
  ...
  useEffect(() => {
    if (isEdit && id) {
      const found = mockLevelDampak.find((a) => a.id === id);
      if (found) { setNama(found.nama); setSkor(found.skor); setDeskripsi(found.deskripsi); }
      else { alert("Data tidak ditemukan"); navigate("/risiko/dampak"); }
    }
  }, [id, isEdit, navigate]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEdit && id) {
      const idx = mockLevelDampak.findIndex((a) => a.id === id);
      if (idx !== -1) mockLevelDampak[idx] = { id, nama, skor: Number(skor), deskripsi };
    } else {
      mockLevelDampak.push({ id: String(Date.now()), nama, skor: Number(skor), deskripsi });
    }
    ...
    <>
      ...
      <div className="max-w-xl space-y-6">
        <h2 className="text-base font-semibold text-gray-800 dark:text-white/90">
          {isEdit ? "Edit Level Dampak" : "Tambah Level Dampak"}
        </h2>
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button type="button" variant="outline" size="sm" className="gap-1.5" onClick={() => navigate("/risiko/dampak")}>
                <CloseIcon /> Batal
              </Button>
            </div>
          </form>
