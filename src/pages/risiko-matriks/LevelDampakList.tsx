...
export default function LevelDampakList() {
  ...
  const [data, setData] = useState<LevelDampak[]>([]);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await levelDampakService.getPaginated({
  ...
  const handleDelete = async (id: string, nama: string) => {
    ...
    try { await levelDampakService.delete(id); fetchData(); }
  ...
  const columns: DataTableColumn<LevelDampak>[] = [
    { key: "skor", header: "Skor", headerClassName: "w-20", render: (item) => (
      <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-warning-50 dark:bg-warning-900/20 text-warning-600 dark:text-warning-400 font-bold text-sm">{item.skor}</span>
    )},
    ...
    <>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <Button size="sm" onClick={() => navigate("/risiko/dampak/create")} className="gap-1.5"><PlusIcon /> Tambah</Button>
        </div>
