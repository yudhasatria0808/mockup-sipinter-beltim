...
import { levelKemungkinanService } from "../../services/masterDataService";

export default function LevelKemungkinanList() {
  const navigate = useNavigate();
  const [data, setData] = useState<LevelKemungkinan[]>([]);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await levelKemungkinanService.getPaginated({
  ...
  const handleDelete = async (id: string, nama: string) => {
    ...
    try { await levelKemungkinanService.delete(id); fetchData(); }
    catch { alert("Gagal menghapus. Pastikan tidak digunakan di matriks."); }
  };

  const columns: DataTableColumn<LevelKemungkinan>[] = [
    { key: "skor", header: "Skor", headerClassName: "w-20", render: (item) => (
      <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-brand-50 dark:bg-brand-900/20 text-brand-600 dark:text-brand-400 font-bold text-sm">{item.skor}</span>
    )},
    ...
    <>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <Button size="sm" onClick={() => navigate("/risiko/kemungkinan/create")} className="gap-1.5"><PlusIcon /> Tambah</Button>
        </div>
